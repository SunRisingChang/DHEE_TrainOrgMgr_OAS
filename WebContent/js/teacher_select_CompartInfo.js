...
$(document).ready(function () {
	...
	but1.click(function () {
		if (a.val() === "" && b.val() === "") {
			layer.tips("请输入企业名称或企业联系人!", a, {
				tips: [1, '#78BA32']
			});
		} else {
			$.ajax({
				url: 'admin_selectComparyInfo.action',
				type: 'POST',
				data: {
					"className": a.val(),
					"studentName": b.val(),
				...
				success: function (data, textStatus, jqXHR) {
					if (data === 110) {
						...
						a.attr("name", data[0].comName);
						b.attr("name", data[0].comMan);
						...
						$.each(data[0].pagelist, function (index, value) {
							$("#table1 tbody").append('<tr><td style="text-align: center;width: 8px;">' + (data[0].nowpage * 10 + 1 + index) + '</td><td style="text-align: center;width: 140px;">' + value.comName + '</td><td style="text-align: center;width: 50px;">' + value.comMan + '</td><td style="text-align: center;width: 55px;">' + value.zhiWu + '</td><td style="text-align: center;width: 8px;">' + value.zuoJi + '</td><td style="text-align: center;width: 8px;">' + value.shouJi + '</td><td style="text-align:center;width: 71px;"><q id="xiangqing" class="q1" rowid="' + index + '">详情</q><q id="shanchu" class="q2" rowid="' + value.id + '">删除</q></td></tr>');
						});
						src_json = data;
	...
	but2.click(function () {
		layer.open({
			...
			fixed: true, //不固定
			maxmin: false,
			content: 'jsp/teacher/teacher_addComparyInfo.jsp'
		});
	});
	...
	$("#table1").on("click", "#shanchu", function () {
		...
		layer.confirm('您确认要删除该条记录？', {
			...
			$.ajax({
				url: 'admin_deleteComparyInfo.action',
				type: 'POST',
				data: {
	...
	$("#table1").on("click", "#xiangqing", function () {
		...
		layer.open({
			...
			fixed: true, //不固定
			maxmin: false,
			content: 'jsp/teacher/teacher_showEditComparyInfo.jsp'
		});
	...
	but3.click(function () {
		if (a.attr("name") === "" && b.attr("name") === "") {
			...
			form.attr("action", "adminInit_outComparyInfoExl.action");
			var input1 = $("<input>");
